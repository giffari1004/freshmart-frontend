...
import { useCreateVoucher } from "@/features/discount/voucher/hooks";
import { CREATE_VOUCHER } from "@/features/discount/voucher/schema";
import { VOUCHER_USAGE_TYPE, VOUCHER_VALUE_TYPE } from "@/features/discount/voucher/constant";
...
import { createVoucherInput, createVoucherOutput } from "@/features/discount/voucher/schema";
...
export function CreateVoucher() {
  ...
  const form = useForm<createVoucherInput, any, createVoucherOutput>({
    ...
    defaultValues: { code: "", usageType: "CART_TOTAL", valueType: "PERCENTAGE", value: 0, isActive: true },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-green-700 hover:bg-green-800">Create voucher</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[520px] rounded-3xl p-6 border-green-200">
        <DialogHeader className="space-y-2">
          <DialogTitle className="text-3xl font-bold tracking-tight">Create voucher</DialogTitle>
        ...
        <form onSubmit={form.handleSubmit(onSubmitButton)} className="space-y-5 pt-2">
          <div className="space-y-2">
            ...
            <Input id="code" className="h-12 rounded-2xl" placeholder="e.g. FRESH10" {...form.register("code")} />
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => form.setValue("usageType", value as createVoucherOutput["usageType"])}
            >
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => form.setValue("valueType", value as createVoucherOutput["valueType"])}
            >
              <SelectTrigger className="w-full h-12 rounded-2xl">
          ...
          <div className="space-y-2">
            ...
            {form.formState.errors.maxDiscountAmount && (
              <p className="text-destructive text-xs">{form.formState.errors.maxDiscountAmount.message}</p>
            )}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Controller
              ...
              name="expiredAt"
              render={({ field }) => (
                <Input id="expiredAt" type="date" className="h-12 rounded-2xl" onChange={(e) => field.onChange(new Date(e.target.value))} />
              )}
            />
            {form.formState.errors.expiredAt && (
              <p className="text-destructive text-xs">{form.formState.errors.expiredAt.message}</p>
            )}
          </div>
          <Button
            type="submit"
