import { api } from "@/lib/axios";
import {
  getCategoryReportSchema,
  getMonthlyReportSchema,
  getProductReportSchema,
} from "./schema";
import { dataTypeGetCategoryReport, dataTypeGetProductReport } from "./constant";

export async function fetchMonthlyReport(query: getMonthlyReportSchema) {
  const { data } = await api.get("/reports/sales/monthly", { params: query });
  return data.data;
}
export async function fetchProductReport(query: getProductReportSchema):Promise<dataTypeGetProductReport[]> {
  const { data } = await api.get("/reports/sales/product", { params: query });
  return data.data;
}
export async function fetchCategoryReport(query: getCategoryReportSchema):Promise<dataTypeGetCategoryReport[]> {
  const { data } = await api.get("/reports/sales/category", { params: query });
  return data.data;
}
